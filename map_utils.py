...
class GameMap(Map):
    def __init__(self, width, height):
        ...
        self.targeted = [[False for y in range(height)] for x in range(width)]

def set_highlight(game_map, x, y):
    if (game_map.walkable[x, y] == True):
        game_map.highlight[x][y] = True

def set_targeted(game_map, x, y):
   game_map.targeted[x][y] = True
   return True

def reset_flags(game_map):
